...
import { 
  Server, Layers, Brain, Mail, ShieldCheck, CheckCircle2, 
  ExternalLink, ArrowUpRight, Activity, Terminal, Send, Eye, Edit3, ScrollText, Check
} from 'lucide-react';
...
export default function Projects() {
  ...
  return (
    <section id="projects" className="relative py-28 border-t border-white/[0.06] bg-ink-950/80 overflow-hidden">
      {/* Precision grid backdrop */}
      <div className="absolute inset-0 grid-bg-enterprise opacity-25" />
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto px-6">
        ...
        <div className="reveal reveal-delay-1 glass-card rounded-3xl p-8 sm:p-12 border-glow-hover relative overflow-hidden">
          {/* Case Study Header Banner */}
          <div className="flex flex-wrap items-center justify-between gap-4 pb-8 mb-8 border-b border-white/[0.08]">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className="font-mono text-xs px-2.5 py-0.5 rounded-full bg-brand-500/10 text-brand-400 border border-brand-500/20 font-medium">
                  CASE STUDY #01
                </span>
                <span className="font-mono text-xs px-2.5 py-0.5 rounded-full bg-accent-500/10 text-accent-400 border border-accent-500/20 font-medium">
                  ENTERPRISE MVP
                </span>
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              {['Medallion Pipeline', 'RAG & Text-to-SQL', 'Concept Rules', 'Human-in-the-Loop'].map((tag) => (
                <span key={tag} className="font-mono text-[11px] px-3 py-1 rounded-lg bg-ink-900/90 text-ink-300 border border-white/[0.06]">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
            {METRICS.map((m) => (
              <div key={m.label} className="p-5 rounded-2xl bg-ink-900/60 border border-white/[0.06]">
                <div className="font-mono text-3xl font-bold text-brand-400 tracking-tight">{m.value}</div>
                <div className="font-display text-xs font-semibold text-white mt-1 uppercase tracking-wider">{m.label}</div>
                <div className="text-xs text-ink-400 mt-0.5">{m.detail}</div>
              </div>
            ))}
          </div>
          ...
          {activeTab === 'architecture' && (
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-fade-in">
              {FLOW_STEPS.map((step) => {
                ...
                return (
                  <div key={step.step} className="p-6 rounded-2xl bg-ink-900/50 border border-white/[0.05] flex flex-col justify-between">
                    <div>
                      <div className="flex items-center justify-between mb-4">
                        <span className="font-mono text-xs font-bold text-brand-400">{step.step}</span>
                        <div className="w-8 h-8 rounded-lg bg-brand-500/10 border border-brand-500/20 flex items-center justify-center text-brand-400">
                          <Icon className="w-4 h-4" />
                        </div>
                      </div>
                      <h4 className="font-display font-bold text-white text-base">{step.title}</h4>
                      <p className="font-mono text-xs text-brand-400/90 mt-0.5 mb-3">{step.subtitle}</p>
                      <p className="text-xs text-ink-300 leading-relaxed">{step.desc}</p>
                    </div>
                  </div>
                );
              })}
          ...
          {activeTab === 'hil' && (
            <div className="p-6 rounded-2xl bg-ink-900/60 border border-white/[0.06] animate-fade-in">
              <div className="grid lg:grid-cols-12 gap-6 items-center">
                <div className="lg:col-span-7 space-y-4">
                  <div className="flex items-center justify-between pb-3 border-b border-white/[0.06]">
                    <div className="flex items-center gap-2">
                      <Terminal className="w-4 h-4 text-brand-400" />
                      <span className="font-mono text-xs text-white font-semibold">TICKET INSPECTION #TKT-4821</span>
                    </div>
                  ...
                  <div className="space-y-3 font-mono text-xs">
                    <div className="p-3.5 rounded-lg bg-ink-950/70 border border-white/[0.04] space-y-1 text-ink-200">
                      <div className="text-ink-400 text-[11px]">// Alert Inference Result</div>
                      <div>Rack ID: <span className="text-white">Rack-A12 (Zone B)</span></div>
                      <div>SLA Condition: <span className="text-amber-400">Temperature &gt; 35°C (3x within 24h)</span></div>
                      <div>Action: <span className="text-brand-300">Drafted Customer Notification &amp; Tech Dispatch</span></div>
                    </div>

                    <div className="p-3.5 rounded-lg bg-ink-950/70 border border-white/[0.04] space-y-1">
                      <div className="text-ink-400 text-[11px]">// AI Drafted Notice Preview</div>
                ...
                <div className="lg:col-span-5 flex flex-col justify-between gap-4 p-5 rounded-xl bg-ink-950/90 border border-white/[0.08]">
                  <div className="space-y-2">
                    <h5 className="font-display text-sm font-bold text-white">Ops Control Action</h5>
                  ...
                  <div className="space-y-2 pt-2">
                    <button
                      ...
                      className={`w-full py-2.5 px-4 rounded-lg font-mono text-xs font-semibold flex items-center justify-center gap-2 transition-all ${
                        approvedState
                    ...
                    {approvedState && (
                      <p className="font-mono text-[11px] text-accent-400 text-center animate-fade-in">
          ...
          {activeTab === 'standards' && (
            <div className="grid sm:grid-cols-3 gap-4 animate-fade-in">
              <div className="p-5 rounded-2xl bg-ink-900/50 border border-white/[0.05]">
                <span className="font-mono text-[10px] px-2 py-0.5 rounded bg-brand-500/10 text-brand-400 font-semibold uppercase">Data Architecture</span>
                <h4 className="font-display font-bold text-white text-base mt-2 mb-1">Databricks Medallion</h4>
              ...
              <div className="p-5 rounded-2xl bg-ink-900/50 border border-white/[0.05]">
                <span className="font-mono text-[10px] px-2 py-0.5 rounded bg-accent-500/10 text-accent-400 font-semibold uppercase">Cloud &amp; AI Security</span>
                <h4 className="font-display font-bold text-white text-base mt-2 mb-1">AWS GenAI Best Practices</h4>
                <p className="text-xs text-ink-300 leading-relaxed">
              ...
              <div className="p-5 rounded-2xl bg-ink-900/50 border border-white/[0.05]">
                <span className="font-mono text-[10px] px-2 py-0.5 rounded bg-amber-500/10 text-amber-400 font-semibold uppercase">ITSM Standard</span>
                <h4 className="font-display font-bold text-white text-base mt-2 mb-1">ITIL Framework Compliance</h4>
                <p className="text-xs text-ink-300 leading-relaxed">
