...
export default function Hero() {
  ...
  return (
    <section id="hero" className="relative min-h-[92vh] flex items-center pt-32 pb-20 overflow-hidden">
      ...
      <div className="absolute inset-0 grid-bg-enterprise opacity-60 dark:opacity-40" />
      <div className="absolute inset-0 bg-gradient-to-b from-background/20 via-background/80 to-background pointer-events-none" />

      {/* Atmospheric glow */}
      <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] bg-brand-500/10 dark:bg-brand-500/10 rounded-full blur-[140px] pointer-events-none" />
      ...
      <div className="relative max-w-7xl mx-auto px-6 grid lg:grid-cols-12 gap-12 items-center w-full">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          ...
          <div className="inline-flex items-center gap-2.5 px-3.5 py-1.5 rounded-full glass-pill w-fit animate-fade-in">
            ...
            <span className="font-mono text-xs font-medium text-brand-700 dark:text-brand-300 tracking-wide uppercase">
          ...
          <p className="text-base sm:text-lg text-muted leading-relaxed max-w-2xl animate-fade-up" style={{ animationDelay: '0.1s' }}>
            เราออกแบบและพัฒนาโซลูชัน <strong className="text-foreground font-semibold">AI, Machine Learning</strong> และ <strong className="text-foreground font-semibold">Data Pipeline</strong> ที่ฉลาดขึ้นทุกครั้งที่มีข้อมูลใหม่เข้ามา ขับเคลื่อนด้วยแนวคิด <span className="font-mono text-xs px-2 py-0.5 rounded bg-brand-500/10 border border-brand-500/20 text-brand-700 dark:text-brand-300 font-medium">T-P-E (Task, Performance, Experience)</span> สู่ระบบระดับ Enterprise ที่ใช้งานได้จริงและวัดผลได้อย่างแม่นยำ
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3.5 pt-2 animate-fade-up" style={{ animationDelay: '0.18s' }}>
            ...
            <button
              ...
              className="px-6 py-3.5 glass-pill hover:bg-surface-hover text-foreground font-semibold text-sm rounded-xl flex items-center justify-center gap-2 transition-all duration-200"
            >
          ...
          <div className="grid grid-cols-3 gap-4 pt-6 border-t border-border animate-fade-up" style={{ animationDelay: '0.25s' }}>
            <div className="flex items-center gap-2.5">
              <Workflow className="w-4 h-4 text-brand-600 dark:text-brand-400 flex-shrink-0" />
              <span className="text-xs font-medium text-muted">Well-Posed ML (T-P-E)</span>
            </div>
            <div className="flex items-center gap-2.5">
              <Layers className="w-4 h-4 text-accent-600 dark:text-accent-400 flex-shrink-0" />
              <span className="text-xs font-medium text-muted">Medallion Architecture</span>
            </div>
            <div className="flex items-center gap-2.5">
              <ShieldCheck className="w-4 h-4 text-brand-600 dark:text-brand-400 flex-shrink-0" />
              <span className="text-xs font-medium text-muted">Human-in-the-Loop</span>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 animate-fade-up" style={{ animationDelay: '0.3s' }}>
          <div className="glass-card rounded-2xl p-6 glow-cyan-enterprise relative overflow-hidden shadow-xl">
            {/* Top Bar */}
            <div className="flex items-center justify-between pb-4 mb-4 border-b border-border">
              <div className="flex items-center gap-2.5">
                <div className="w-2.5 h-2.5 rounded-full bg-brand-500 dark:bg-brand-400 animate-pulse" />
                <span className="font-mono text-xs font-semibold text-foreground tracking-wide">
                  EXPERIENCE ENGINE
                </span>
              </div>
              <span className="font-mono text-[11px] px-2.5 py-0.5 rounded-full bg-accent-500/10 text-accent-700 dark:text-accent-400 border border-accent-500/20 font-medium">
                SYSTEM ONLINE
              </span>
            </div>

            {/* Live ML Philosophy Terminal Block */}
            <div className="bg-surface-muted rounded-xl p-4 border border-border mb-4 font-mono text-xs space-y-2">
              <div className="flex items-center justify-between text-muted border-b border-border pb-2">
                ...
                <span className="text-[10px] text-brand-600 dark:text-brand-400 font-semibold">Mitchell (1997)</span>
              </div>
              <p className="text-foreground leading-relaxed">
                <span className="text-brand-600 dark:text-brand-400 font-semibold">T</span>ask: Automated Decision &amp; SLA Prediction<br />
                <span className="text-accent-600 dark:text-accent-400 font-semibold">P</span>erformance: Accuracy &gt; 99.4%, SLA Latency &lt; 0.3s<br />
                <span className="text-amber-600 dark:text-amber-400 font-semibold">E</span>xperience: Continuous Ingestion &amp; Version Space
              </p>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] text-brand-600 dark:text-brand-400 font-bold">{item.step}</span>
                      <span className={isActive ? 'text-foreground font-medium' : 'text-muted'}>{item.stage}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-foreground font-medium">{item.metric}</span>
                      <span className={`text-[10px] px-2 py-0.5 rounded ${
                        isActive ? 'bg-accent-500/20 text-accent-700 dark:text-accent-300 font-semibold' : 'text-muted'
                      }`}>
                        {item.status}
                      </span>
                    </div>
            ...
            <div className="pt-3 border-t border-border flex items-center justify-between text-xs text-muted">
              ...
              <span className="font-mono text-foreground text-[11px]">24/7 Verified</span>
            </div>
          </div>
        </div>
      </div>
